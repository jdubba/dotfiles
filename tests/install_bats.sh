...
if [ ! -d "tests/lib/bats-core" ]; then
  ...
  git clone --depth 1 -b ${BATS_CORE_VERSION} https://github.com/bats-core/bats-core.git tests/lib/bats-core 2>/dev/null || \
  git clone --depth 1 https://github.com/bats-core/bats-core.git tests/lib/bats-core
fi

# Install bats-support
if [ ! -d "tests/lib/bats-support" ]; then
  echo "Installing bats-support..."
  git clone --depth 1 -b ${BATS_SUPPORT_VERSION} https://github.com/bats-core/bats-support.git tests/lib/bats-support 2>/dev/null || \
...
if [ ! -d "tests/lib/bats-assert" ]; then
  ...
  git clone --depth 1 -b ${BATS_ASSERT_VERSION} https://github.com/bats-core/bats-assert.git tests/lib/bats-assert 2>/dev/null || \
