...
EOF
  ...
  # @bar-fg is the bar's own text -- clock, module labels, anything not inside a
  # pill -- drawn on @bar-bg. The palette foreground clears AA against the
  # background by a wide margin on 39 of the 40 shipped themes, but not on
  # solarized-light, whose fg #657b83 is a deliberately soft slate against a
  # cream page: 4.12:1, which is fine for a paragraph of prose and not for
  # single glyphs at bar size. Ramp toward the surface's own contrast ink only
  # as far as AA requires, so every theme that already passes keeps its exact
  # foreground and this reaches the one that does not.
  local bar_fg
  bar_fg=$(_dfa_ramp_to "$fg" "$(_dfa_contrast_fg "$bg")" "$bg" 4500)
  cat >"$dest/.config/waybar/colors.css" <<EOF
/* ${tag} waybar palette */
@define-color bar-bg          ${bg};
@define-color bar-fg          ${bar_fg};
...
EOF
  ...
  # meter_bg and div_line are chrome: the unfilled part of a meter and the rules
  # between boxes, both of which must recede. btop_neutral is a palette slot, not
  # a measurement, and on 15 palettes it lands as a heavy ink rather than a
  # surface -- dawnfox and rose-pine-dawn draw their dark plum #575279 at 6.65:1
  # across a cream canvas, kanagawa-dragon a light grey at 7.34:1 across a
  # near-black one. The light/dark branch above was itself an attempt to fix this
  # and does not help when c7 is dark, which it is whenever c7 doubles as the
  # foreground.
  #
  # Keep the slot wherever it already recedes, so themes that are fine keep their
  # exact colours, and derive a surface only where it does not: mixing the
  # background toward the foreground lifts a dark theme and deepens a light one,
  # so one formula serves both polarities.
  #
  # This is deliberately NOT applied to download_start/upload_start below, which
  # also read btop_neutral: those are graph gradient stops and want to be seen.
  local btop_surface=$btop_neutral
  if (( $(_dfa_contrast "$btop_surface" "$bg") > 2500 )); then
    btop_surface=$(_df_theme_mix "$bg" "$fg" 20)
  fi
  # A btop meter draws its gradient along its own length, so the colour at the
  # filled tip is what reports the level. That makes each gradient a statement
  # about which end of the scale is bad, and used/available are opposites: a
  # nearly-full `used` bar is the alarm, a nearly-full `available` bar is the
  # reassurance. Both were emitted ramping the same direction -- `available` ran
  # c3 -> c5 -> c1, ending on the palette's red, so plenty of free memory lit up
  # exactly like exhaustion; `used` ran c3 -> c1 -> c5 and put its red in the
  # MIDDLE, so a bar got calmer as it filled past half.
  #
  # They now mirror each other on the same three slots, matching what the
  # gruvbox and carbonfox overrides already do by hand and what upstream btop's
  # own everforest themes do (#f85552 -> #a7c080 for available, reversed for
  # used). temp and cpu keep c2 -> c3 -> c1 -- same shape, same meaning.
...
theme[graph_text]="${fg}"
theme[meter_bg]="${btop_surface}"
theme[proc_misc]="${c4}"
theme[cpu_box]="${c4}"
theme[mem_box]="${c2}"
theme[net_box]="${c1}"
theme[proc_box]="${c6}"
theme[div_line]="${btop_surface}"
theme[temp_start]="${c2}"
theme[temp_mid]="${c3}"
theme[temp_end]="${c1}"
theme[cpu_start]="${c2}"
...
theme[cached_mid]="${c4}"
theme[cached_end]="${c5}"
theme[available_start]="${c1}"
theme[available_mid]="${c3}"
theme[available_end]="${c2}"
theme[used_start]="${c2}"
theme[used_mid]="${c3}"
theme[used_end]="${c1}"
